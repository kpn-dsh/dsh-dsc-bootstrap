...
/**
  * Creates and trains Linear Regression model to predict wait time in 15 minutes:
  * - Creates basetable
  * - Creates and configures ML pipeline
  * - Fits the pipeline to the training data
  * - Saves the model
  */
object WaitTime15mModeler extends Serializable {
  def createModel(data: DataFrame, printOutput: Boolean): Unit = {
    ...
    val baseTable = data
      .withColumn("label", lead(col("avgQueueDuration"), 15, null).over(Window.orderBy("pubTime")))
      .na.drop("any")
      .drop("pubTime")
    ...
    pipelineModel.write.overwrite().save("../model-scoring-processor/src/main/resources/pipeline_model_wait_time_15min")
