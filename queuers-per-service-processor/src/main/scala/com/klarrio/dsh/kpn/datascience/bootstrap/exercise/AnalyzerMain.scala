...
object AnalyzerMain {
  def main(args: Array[String]): Unit = {
     ...
     * 3. COMPUTE KPIs: QUEUERS PER SERVICE
      ...
      * 1. Filter out all the callers that are in a queue using dt_start and dt_offered_queue
      * 2. Use reduceByKey to compute the amount of queuers per service
      ...
      *     kafka key: "queuers-per-service"
      *     kafka value:
      *     Create a JSON like this for each service:
      *     "{\"pubTime\":" + pubTime + ",\"serviceName\":\"" + serviceName + "\",\"amtOfQueuersInService\":" + amtOfQueuers.toInt + "}"
      *     put all these together in one list and send that as the kafka value of the observation
